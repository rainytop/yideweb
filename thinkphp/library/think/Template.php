    ...
    private function compiler(&$content, $cacheFile)
    {
        ...
        if ($this->config['layout_on']) {
            if (false !== strpos($content, '{__NOLAYOUT__}')) {
                ...
                // 读取布局模板
                $layoutName= $this->config['layout_name'];
//                trace($layoutName);
//                trace('3');
                $layoutFile = $this->parseTemplateFile($layoutName);
                if ($layoutFile) {
                    // 替换布局的主体内容
    ...
    private function parseTemplateFile($template)
    {
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
            ...
            $viewSuffix= ltrim($this->config['view_suffix'], '.');

            $template = $path . $template . '.' . $viewSuffix;
